...
for (const [name,num,label,right] of [
  ...
  ['02-stack','02','things i use','the toolbox'],
  ['03-build','03','what i like building','side quests'],
  ['04-public','04','out in the open','public repos'],
  ['05-activity','05','leaving a trail','a day at a time'],
]) {
...
<g class="c">${text(24,209,'reason',15,muted)}${text(165,209,'just because',16)}</g>
${text(24,259,'$ make something',16,rust)}<rect class="cursor" x="215" y="245" width="10" height="18" fill="${rust}"/>`, 'law at the terminal');
...
for (const [file,title,tag,lines,lang] of [
 ['zzz','zzz','my project',['real gifs. ascii frames.','a terminal with a little life.'],'python'],
...
await save('footer',1000,80,`<style>@keyframes drift{to{stroke-dashoffset:-40}}.line{animation:drift 5s linear infinite}@media(prefers-reduced-motion:reduce){.line{animation:none}}</style><rect width="1000" height="80" rx="6" fill="${ink}"/><path class="line" d="M24 20H976" stroke="${rust}" stroke-dasharray="2 18"/>${text(24,56,'thanks for stopping by',14,muted)}${text(976,58,'law',26,paper,'text-anchor="end" font-family="Georgia,serif" font-style="italic"')}`,'thanks for stopping by / law');
